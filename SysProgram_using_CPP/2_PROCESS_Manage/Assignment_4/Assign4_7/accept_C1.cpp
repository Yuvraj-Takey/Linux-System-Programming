...
void Accept::count_Capital(const char* cIn_File, const char* cOut_File)
{
	...
	{
		cout<<"[CHILD_1]: "<<cIn_File<<" -file opened for Input"<<endl;
	}
	...
	{
		cout<<"[CHILD_1]: "<<cOut_File<<" -file opened for output"<<endl;
	}	
	...
	// manage string
	sprintf(cOutput_Data,"[CHILD_1]: Number of Capital characters are->  %d",iCount);

	// Writing result into output file
	if((iRet_Val = write(out_FD,cOutput_Data,strlen(cOutput_Data))) == STATUS_ERR)
	{
		cout<<"[CHILD_1]: Unable to write into output file"<<endl<<"Reason: "<<strerror(errno)<<endl;
	}
...
{
	cout<<"[CHILD_1] Hello, Inside Process_1"<<endl;
	
	Accept Obj;
	Obj.count_Capital("demo","count");

	cout<<"[CHILD_1] Exiting.."<<endl;
	return STATUS_SUC;
}
